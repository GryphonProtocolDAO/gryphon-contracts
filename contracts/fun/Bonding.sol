...
import "../persona/IAgentFactory.sol";
import "./FERC20.sol";
import "./FFactory.sol";
import "./FRouter.sol";
import "./IFPair.sol";
...
OwnableUpgradeable
{
    ...
    function sell(
        ...
        uint256 volume = duration > DAY_IN_SECONDS
            ? amount1Out
            : tokenInfo[tokenAddress].data.volume24H + amount1Out;
        uint256 prevPrice = duration > DAY_IN_SECONDS
    ...
    function buy(
        ...
        uint256 _price = duration > DAY_IN_SECONDS
            ? tokenInfo[tokenAddress].data.price
            : tokenInfo[tokenAddress].data.prevPrice;
...
//    function debugOpenTradingOnUniswap(address tokenAddress) external {
//        _openTradingOnUniswap(tokenAddress);
//    }
    ...
    function _openTradingOnUniswap(address tokenAddress) private {
        ...
        uint256 id = IAgentFactory(agentFactory).initFromBondingCurve(
            string.concat(_token.data._name, " by Gryphon"),
            _token.data.ticker,
            _token.cores,
            assetBalance,
            _token.creator
        ...
        address agentToken = IAgentFactory(agentFactory)
            .executeBondingCurveApplication(id,
    ...
    function unwrapToken(
        ...
        for (uint i = 0; i < accounts.length; i++) {
            ...
            uint256 agentPairBalance = agentToken.balanceOf(pairAddress);
            require(balance <= agentPairBalance, "Insufficient pair agent token balance");
            if (balance > 0) {
