...
import "../pool/extuniv2/IExtRouterV2.sol";
import "../pool/extuniv2/IExtFactoryV2.sol";
...
Ownable2StepUpgradeable
{
    ...
    IExtRouterV2 internal _uniswapRouter;
    ...
    function initialize(
        ...
        _uniswapRouter = IExtRouterV2(integrationAddresses_[1]);
    ...
    function _createPair() internal returns (address uniswapV2Pair_) {
        address uniswapFactory = _uniswapRouter.factory();
        uniswapV2Pair_ = IExtFactoryV2(uniswapFactory).getPair(
            address(this),
            pairToken,
            false
        );
        ...
        if (uniswapV2Pair_ == address(0)) {
            uniswapV2Pair_ = IExtFactoryV2(uniswapFactory)
                .createPair(address(this), pairToken, false);
...
//        path[0] = address(this);
//        path[1] = pairToken;
        IExtRouterV2.route[] memory routes = new IExtRouterV2.route[](1);
        routes[0] = IExtRouterV2.route({
            from: address(this),
            to: pairToken,
            stable: false
        });
